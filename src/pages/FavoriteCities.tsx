      ...
      <div className="layout-container flex h-full grow flex-col">
        ...
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-gray-700 px-6 md:px-10 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="flex items-center justify-center rounded-full size-10 bg-gray-700">
          ...
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1 gap-8">
            <div className="flex flex-wrap justify-between items-center gap-4">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-base font-normal leading-normal text-gray-400">Your handpicked list of cities for quick weather checks.</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {/* Sprawdzamy, czy są jakieś ulubione */}
              {favoriteCities.length > 0 ? (
                favoriteCities.map(city => (
                  <FavoriteCityCard key={city.id} city={city} />
                ))
              ) : (
                // "Empty State" z Twojego HTML (był zakomentowany)
                <div className="col-span-full flex flex-col items-center justify-center text-center gap-4 p-10 bg-card-dark rounded-lg">
                  <span className="material-symbols-outlined text-gray-500 text-6xl">
                    star
                  </span>
                  <h3 className="text-2xl font-bold">You have no favorite cities yet.</h3>
                  <p className="text-gray-400">Click the star icon on the main screen to add a city to your favorites.</p>
                  <Link 
                    to="/"
        ...
        {/* Footer z Twojego HTML */}
        <footer className="w-full border-t border-solid border-gray-700 mt-auto">
          <div className="max-w-[960px] mx-auto px-6 md:px-10 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <p className="text-sm text-gray-400">© 2024 WeatherApp</p>
          </div>
        </footer>
      </div>
    </div>
